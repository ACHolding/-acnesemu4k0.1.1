...
class ACNES:
    ...
    def cpu_read(self, addr):
        ...
        if addr < 0x4000:
            if addr == 0x2002:  # PPUSTATUS - fake vblank for commercial ROM boot
                val = 0
                if self.ppu_vblank_flag:
                    val |= 0x80
                    self.ppu_vblank_flag = False
                return val
        ...
        if addr >= 0x8000 and self.cart.valid:
            if self.cart.mapper == 2:  # UxROM - common commercial mapper
                bank_size = 16384
                num_banks = len(self.cart.prg) // bank_size
                if addr < 0xC000:
                    bank = self.cart.prg_bank & (num_banks - 1)
                    return self.cart.prg[bank * bank_size + (addr - 0x8000)]
                else:
                    last = num_banks - 1
                    return self.cart.prg[last * bank_size + (addr - 0xC000)]
    ...
    def cpu_write(self, addr, value):
        ...
        if addr < 0x2000:
            ...
            if value & 1:
                ...
                self.controller_strobe = False
                self.controller_shift = self.controller
        elif addr >= 0x8000 and self.cart.valid:
            if self.cart.mapper == 2:  # UxROM PRG bank select
                self.cart.prg_bank = value & 0x0F
    ...
    def render_pixels(self):
        ...
        num_tiles = max(1, len(chr_data) // 16)
        for y in range(NES_H):
            row = []
            for x in range(NES_W):
                tile_x = x // 8
...
class FCEUXStyleGUI:
    ...
    def about(self):
        messagebox.showinfo(
            ...
            "Supports mapper 0 (NROM) and mapper 2 (UxROM).\n"
            "CPU: 6502 + common ops. PPU: fake vblank + CHR viewer.\n"
